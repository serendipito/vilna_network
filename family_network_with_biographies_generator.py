# This file is responsible for generating family network data (with biographical data).
# Make sure you have biographies.json ready
...
biography_file = "biographies.json"
with open(biography_file) as json_file:
...
for member in members:
	...
	if include_biographies:
		try:
			# we only include the first sentence
			summary = biographies[member].split('.', 1)[0]
		except:
			summary = ''

	# we remove text in parentheses
